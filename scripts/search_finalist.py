...
def main():
    ap = argparse.ArgumentParser(description="Finalist study: jointly tune sensitive Step1+Step2 params")
    ...
    study_name = args.study if args.study else f"finalist_{task}"
    if args.db:
        storage = args.db
    else:
    ...
    def suggest(trial: optuna.Trial) -> Dict[str, Any]:
        ...
        # sample combined sensitive params
        ...
        run_dir = proj_root / "results" / "optuna" / study_name / f"{ts}_{task}_eeg_finalist_t{trial.number:03d}"
    ...
    out_dir = proj_root / "results" / "optuna_studies" / task / "finalist"
    out_dir.mkdir(parents=True, exist_ok=True)
    (out_dir / "best.json").write_text(json.dumps(best_payload, indent=2))
    print("Finalist best:", best_payload)
