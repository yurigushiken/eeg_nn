...
class TrainingRunner:
    ...
    def run(self, dataset, groups, class_names, model_builder, aug_builder, input_adapter=None, optuna_trial=None):
        ...
        # Split strategy: n_folds or LOSO
        ...
        for fold, (tr_idx, va_idx) in enumerate(fold_iter):
            ...
            cls_w = compute_class_weight("balanced", classes=np.arange(num_cls), y=y_all[inner_tr_idx])
            ...
            for epoch in range(1, int(self.cfg.get("epochs", 60)) + 1):
                ...
                if patience >= int(self.cfg.get("early_stop", 10)):
                    break
                if epoch % 5 == 0:
                    print(f"  [epoch {epoch}] tr_loss={train_loss:.4f} va_loss={val_loss:.4f} va_acc={val_acc:.2f} best_val={best_val:.4f}", flush=True)
            ...
            inner_accs.append(best_inner_acc); inner_macro_f1s.append(best_inner_macro_f1)
            print(f"[fold {fold+1}] acc={acc:.2f} inner_best_acc={best_inner_acc:.2f} inner_best_macro_f1={best_inner_macro_f1:.2f}", flush=True)
